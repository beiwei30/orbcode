...
use crate::{AppConfig, EditorModeSetting, ThemeSetting, claude_home::sanitize_path};

/// Three-state runtime model override:
/// `None` means untouched, `Some(None)` means explicit default, and
/// `Some(Some(model))` means an explicit runtime model.
pub type RuntimeModelOverride = Option<Option<String>>;
...
impl RuntimeSessionState {
    ...
    pub fn effective_config(&self, config: &AppConfig) -> AppConfig {
        ...
        if let Some(model) = self.model_override() {
            config.apply_runtime_model_override(model.as_deref());
        }
        config
    }
