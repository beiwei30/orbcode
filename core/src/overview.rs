...
impl ContextCategoryBreakdown {
    ...
    /// Sum of categories that are not user/assistant conversation content —
    /// system prompt, tool schemas, memory files, skills. Preserved so the
    /// legacy coarse `System/Tools` aggregate stays consistent.
    ...
    /// Sum of categories that represent transcript/turn content —
    /// conversation messages plus attachments.
...
/// Per-session cumulative cost surface backing the `/cost` command.
///
/// Unlike [`UsageOverview`], which is recomputed from the persisted transcript
/// on demand, this is produced from the live per-session [`crate::CostTracker`]
/// that accumulates as turns complete and is seeded from the transcript on
/// resume. The two are expected to agree for a session without compaction.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
