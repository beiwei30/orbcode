...
use crate::render::styled_wrap::{
    ViewportProjection, ensure_source_range_visible, wrap_styled_line_preserving_whitespace,
};
...
pub(crate) struct DiffOverlayState {
    ...
    pub(crate) ensure_selection_visible: bool,
...
impl DiffOverlayState {
    ...
    pub(crate) fn cached_visible_lines(&mut self, area: Rect) -> Vec<StyledLine> {
        ...
        self.refresh_file_content_projection(area.width as usize);
        if self.file_content_cache.visual_rows.is_empty() {
        ...
        self.file_content_cache
            .visual_rows
            .iter()
            .skip(self.line_scroll)
            .take(visible_height)
            .cloned()
            .collect()
    }

    fn refresh_file_content_projection(&mut self, available_width: usize) -> bool {
        let mode = self.mode;
        ...
        let projection = diff_file_visual_projection(
            ...
            line_no_width,
            available_width,
            syntax_lines.as_deref(),
            &extension,
        );
        self.file_content_cache.line_no_width = line_no_width;
        self.file_content_cache.extension = extension;
        self.file_content_cache.syntax_lines = syntax_lines;
        self.file_content_cache.visual_rows = projection.visual_rows;
        self.file_content_cache.source_line_by_visual_row = projection.source_line_by_visual_row;
    ...
    pub(crate) fn scroll_visual_rows(&mut self, delta: isize) {
    ...
    pub(crate) fn scroll_lines(&mut self, delta: isize) {
        self.scroll_visual_rows(delta);
    }

    pub(crate) fn move_line_selection(&mut self, delta: isize) {
        let line_count = self.selected_line_count();
        if line_count == 0 {
            self.selected_line = 0;
            return;
        }
    ...
    pub(crate) fn clamp_selection(&mut self) {
        ...
        let selected_line_count = match self.mode {
            DiffOverlayMode::Unstaged => self.files_cache.unstaged_files.as_deref().unwrap_or(&[]),
            DiffOverlayMode::Staged => self.files_cache.staged_files.as_deref().unwrap_or(&[]),
        }
        ...
        self.selected_line = self
            .selected_line
            .min(selected_line_count.saturating_sub(1));
...
pub(crate) fn apply_diff_overlay_key(
    ...
    match key_event.code {
        ...
        KeyCode::PageUp => {
            diff.scroll_visual_rows(-(diff.viewport_height.max(1) as isize));
            DiffOverlayAction::None
        }
        ...
        KeyCode::Home | KeyCode::Char('g') => {
            diff.selected_line = 0;
            diff.line_scroll = 0;
            diff.ensure_selection_visible = true;
            DiffOverlayAction::None
        }
        KeyCode::End | KeyCode::Char('G') => {
            diff.selected_line = diff.selected_line_count().saturating_sub(1);
            diff.ensure_selection_visible = true;
            DiffOverlayAction::None
        }
...
pub(crate) fn sync_diff_overlay_bounds(diff: &mut DiffOverlayState, area: Rect) {
    ...
    let content_area = diff_overlay_content_area(area);
    let visible_height = content_area.height as usize;
    diff.viewport_height = visible_height;
    if diff.refresh_file_content_projection(content_area.width as usize) {
        diff.ensure_selection_visible = true;
    }
    diff.max_line_scroll = diff
        .file_content_cache
        .visual_rows
        .len()
        .saturating_sub(visible_height);
    diff.line_scroll = diff.line_scroll.min(diff.max_line_scroll);
    if diff.ensure_selection_visible && visible_height > 0 {
        diff.line_scroll = ensure_source_range_visible(
            &diff.file_content_cache.source_line_by_visual_row,
            diff.file_content_cache.visual_rows.len(),
            diff.line_scroll,
            visible_height,
            diff.selected_line,
            diff.selected_line,
        ...
        diff.line_scroll = diff.line_scroll.min(diff.max_line_scroll);
        diff.ensure_selection_visible = false;
    }
}

fn diff_overlay_content_area(area: Rect) -> Rect {
    let panel_area = Rect {
...
pub(crate) fn render_diff_line_with_palette(
    ...
    render_diff_visual_rows_with_palette(
        line,
        selected,
        line_no_width,
        available_width,
...
fn diff_file_visual_projection(
    file: &DiffFile,
    selected_line: usize,
    line_no_width: usize,
    available_width: usize,
    syntax_lines: Option<&[Vec<Span<'static>>]>,
    extension: &str,
) -> ViewportProjection {
    let mut projection = ViewportProjection::default();
    for (source_line, line) in file.lines.iter().enumerate() {
        let visual_rows = render_diff_visual_rows_with_palette(
            line,
            source_line == selected_line,
            line_no_width,
            available_width,
            syntax_lines.and_then(|lines| lines.get(source_line).map(Vec::as_slice)),
            extension,
            active_palette(),
        );
        projection
            .source_line_by_visual_row
            .extend(std::iter::repeat_n(source_line, visual_rows.len()));
        projection.visual_rows.extend(visual_rows);
    }
    projection
}

fn render_diff_visual_rows_with_palette(
    line: &DiffRenderLine,
    selected: bool,
    line_no_width: usize,
    available_width: usize,
    ...
    if line.kind == DiffLineKind::Separator {
        ...
        return vec![Line::from(vec![
            ...
            Span::styled(
                "─".repeat(available_width.saturating_sub(1)),
                subtle_style(),
            ),
        ])];
    }
    ...
    let content_spans = if line.kind == DiffLineKind::Note {
        vec![Span::styled(line.content.clone(), subtle_style())]
    } else {
        syntax_spans
            .map_or_else(
    ...
    let gutter_width = line_no_width.saturating_add(7);
    let content_width = available_width.saturating_sub(gutter_width).max(1);
    let wrapped_content =
        wrap_styled_line_preserving_whitespace(&Line::from(content_spans), content_width);

    wrapped_content
        .into_iter()
        .enumerate()
        .map(|(visual_index, content)| {
            let first = visual_index == 0;
            let line_number = if first {
                line.new_line.or(line.old_line).map_or_else(
            ...
            let mut spans = vec![
                ...
                Span::styled(" ", row_style),
                Span::styled(
                    if first { line.marker } else { ' ' }.to_string(),
                    if first { marker_style } else { row_style },
                ),
                Span::styled("  ", row_style),
            ];
